    ...
    >
      ...
      <div className="px-3.5 pt-3 pb-3.5">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-[10px] text-muted">{post.year}</span>
        </div>
        <p className="font-display text-[13px] font-semibold text-white leading-snug group-hover:text-accent transition-colors duration-200 line-clamp-1">{post.title}</p>
        <p className="text-[11px] text-muted mt-0.5 leading-snug line-clamp-2">{post.desc}</p>
      </div>
    </a>
  )
}
  ...
  return (
    <>
      ...
      <section className="px-4 sm:px-8 md:px-12 py-20 md:py-28">
          ...
          {CATS.map(c => {
            ...
            return (
              <button
                ...
                data-hover
                className={`flex items-center gap-1.5 text-[11px] font-semibold tracking-[0.08em] uppercase border px-4 py-2 rounded-full transition-all duration-200 font-display ${
                  activeCat === c
        ...
        <MasonryGrid key={activeCat} posts={filteredPosts} />
